...
bool Object3d::InitializeGraphicsPipeline()
{
	...
	for (int32_t i = 0; i < vsSize; i++) {
		...
		result = DirectXSetting::GetIns()->GetDev()->CreateGraphicsPipelineState(&gpipeline, IID_PPV_ARGS(&pipelinestate_[i]));
...
void Object3d::Update(const float maxTime)
{
	...
	if (SUCCEEDED(constBuffB0_->Map(0, nullptr, (void**)&shadowMap))) {
